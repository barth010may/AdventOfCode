...
steps = 0
path = []
while path1.index != starting_node.index:
    new_node1 = get_next_path(previous_node1, path1)
    previous_node1 = path1.copy()
    path1 = new_node1.copy()
    path.append(path1.index)

# Convert the list of tuples to a string
tuples_string = str(path)

# Write the string to a text file
# Shitty solution, by counting manually the numbers encapsulated
# Got Chatgpt to visualize
with open('output_file.txt', 'w') as file:
    file.write(tuples_string)
